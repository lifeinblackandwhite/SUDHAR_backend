"""
Seed script to populate GOVTOFFICIALS table with initial data.
Run: docker exec infra-backend python -m app.db.seed_officials
"""
...
async def seed_officials():
    async with AsyncSessionLocal() as session:
        ...
        result = await session.execute(text("SELECT COUNT(*) FROM \"GOVTOFFICIALS\""))
        count = result.scalar()
        
        if count > 0:
            print(f"⚠️  GOVTOFFICIALS already has {count} records. Skipping seed.")
            return
        ...
        print(f"✅ Seeded {len(govt_officials)} government officials successfully!")


if __name__ == "__main__":
    asyncio.run(seed_officials())
